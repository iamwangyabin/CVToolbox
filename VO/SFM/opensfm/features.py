...
# 这个是自己实现的深度学习特征点
# 主要是添加一个函数
# ＬＦＮｅｔ就是先把数据库里图片都给他跑出来
# 需要输入的：
#　    ｃｏｎｆｉｇ：ＬＦ－ｎｅｔ跑出来的特征点地址
#      image: 就是一个图片的名字
def extract_features_LFNet(image, config):
    print(image)
    LFnetFeaturePath = config['LFnetFeaturePath']
    print(LFnetFeaturePath)
    logger.debug('Computing LF-net')

    data=np.load(LFnetFeaturePath+image+'.npz')
    desc = data['descs']
    kps = data['kpts']
    ori = data['oris']
    scales = data['scales']
    ...
    logger.debug('done')
    points=[]
    for p, s, o in zip(kps,scales,ori):
        point=np.array([p[0],p[1],s,o])
        points.append(point)
...
def extract_features(color_image, config, mask=None, imageName=None):
    ...
    elif feature_type == 'LF':
        points, desc = extract_features_LFNet(imageName, config)
    else:
