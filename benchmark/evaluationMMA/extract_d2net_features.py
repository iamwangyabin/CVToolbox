...
def topk(kps, scores, descs,k_=cfg.TRAIN.TOPK):
    kpr = np.array([kp for kp in scores])
    kpc = np.array([[0, int(kp[0]), int(kp[1]), 0] for kp in kps])
    idx = np.argsort(kpr)
    if len(kpr) >= k_:
        idx = idx[len(kpr) - k_:]
    return kpc[idx], descs[idx]
...
def benchmark_features():
    ...
    with torch.no_grad():
        ...
        for i_batch, sample_batched in tqdm(enumerate(data_loader, 1)):
            ...
            for thr in rng:
                TPNN, PNN, TPNNT, PNNT, TPNNDR, PNNDR = eval_d2net(model, batch, 1, thr)
                PreNN = TPNN / PNN
                PreNNT = TPNNT / PNNT
