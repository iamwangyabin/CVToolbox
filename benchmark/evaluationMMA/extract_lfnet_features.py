...
def eval_lfnet_model(parsed_batch, DES_THRSH, COO_THRSH):
    im1_data, im1_info, homo12, im2_data, im2_info, homo21, im1_raw, im2_raw,name1,name2,file = (
        parsed_batch
    )
    # import pdb; pdb.set_trace()
    rootdir = "/home/wang/workspace/lf-net-release/dump_feats"
    npz1 = np.load(os.path.join(rootdir,(file[0] + "_" + name1[0]).split('.')[0] + '.jpg.npz'))
    npz2 = np.load(os.path.join(rootdir,(file[0] + "_" + name2[0]).split('.')[0] + '.jpg.npz'))
    ...
    des1 = torch.tensor(desc1,device = device)
    des2 = torch.tensor(desc2,device = device)
...
def parse(batch, device):
    ...
    return im1_data, im1_info, homo12, im2_data, im2_info, homo21, im1_raw, im2_raw, name1, name2, files
...
def benchmark_features():
    ...
    with torch.no_grad():
        ...
        for i_batch, sample_batched in tqdm(enumerate(data_loader, 1)):
            batch = parse(sample_batched, device)
            for thr in rng:
                TPNN, PNN, TPNNT, PNNT, TPNNDR, PNNDR = eval_lfnet_model(batch, 1, thr)
                PreNN = TPNN / PNN
                PreNNT = TPNNT / PNNT
